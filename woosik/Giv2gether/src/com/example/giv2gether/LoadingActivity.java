...
public class LoadingActivity extends Activity {
	...
	ArrayList<Newsfeed> arrNewsfeedList;
	...
	protected void onCreate(Bundle savedInstanceState) {
		...
		arrNewsfeedList = new ArrayList<Newsfeed>();
		...
		timer = new Thread() {
			public void run() {
				try {
					...
					i.putExtra("newsfeedlist", arrNewsfeedList);
	...
	public void insertWishlistData(int webId, String title, int price,
			int wish, String imagePath, String bookmark) {
		dbManager.insertWishlistData(webId, title, price, wish, imagePath,
				bookmark);
	}
	...
	public void insertFWishlistData(int id, String phone, String title,
			int price, int wish, String date, String imagePath,
			String bookmark, String event) {
		dbManager.insertFWishlistData(id, phone, title, price, wish, date,
				imagePath, bookmark, event);
	}
	...
	public void selectFWishAll() {
		...
		while (!result.isAfterLast()) {
			...
			MyFriendsWish myFWish = new MyFriendsWish(id, phone, title, price,
					wish, eventOn, date, imagePath, bookmarkOn, null);
	...
	/*
	 * 1. WPocket Synchronize 2. Get News Feed 3. Get Friends Wish
	 */
	...
	public void SyncWPocket() {
		...
		try {
			JSONObject jObj = new AsyncTaskWPocketSynchronize().execute(
					"http://naddola.cafe24.com/selectMyWishlist.php?email="
							+ settings.getID()).get();
			JSONArray wishlist;
			...
			for (int i = 0; i < wishlist.length(); i++) {
				...
				if (!checkWishlistData(webId)) {
					insertWishlistData(webId, title, price, wish, imagePath,
							bookmark);
				}
	...
	public void GetNewsFeed() {
		...
		// 생일
		try {
			JSONObject jObj = new AsyncCheckNewsFeed().execute(
					"http://naddola.cafe24.com/getNewsFeedDDays.php").get();
			...
			for (int i = 0; i < feed.length(); i++) {
				...
				for (int j = 0; j < arrMyFriendList.size(); j++) {
					if (phone.equals(arrMyFriendList.get(j).getPhone())) {
						if (dday < 16) {
							arrNewsfeedList.add(new Newsfeed(
									Newsfeed.PUT_FRIEND_BIRTH_DDAY, name,
									email, dday));
						}
					}
				}
		...
		try {
			JSONObject jObj = new AsyncCheckNewsFeed().execute(
					"http://naddola.cafe24.com/getNewsFeedEvent.php").get();
			JSONArray feed = jObj.getJSONArray("feed");
			progressBar.setProgress(progress += 10);
			...
			for (int i = 0; i < feed.length(); i++) {
				...
				for (int j = 0; j < arrMyFriendList.size(); j++) {
					if (phone.equals(arrMyFriendList.get(j).getPhone())) {
						if (dday > -3) {
							arrNewsfeedList.add(new Newsfeed(
									Newsfeed.PUT_EVENT_GENERATION, name, email,
									dday));
						}
					}
				}
		...
		progressBar.setProgress(progress += 10);
		/*
		 * // 생일2 try { JSONObject jObj = new AsyncCheckNewsFeed().execute(
		 ...
		 * progressBar.setProgress(progress+=10);
		 * 
		 * // 시작된 이벤트 try { JSONObject jObj = new AsyncCheckNewsFeed().execute(
		 * "http://naddola.cafe24.com/getNewsFeedEvent.php").get(); JSONArray
		 * feed = jObj.getJSONArray("feed");
		 * 
		 * progressBar.setProgress(progress+=10);
		 * 
		 ...
		 * feed.getJSONObject(i);
		 * 
		 * String name = c.getString("name"); String title =
		 * c.getString("title"); String phone = c.getString("phone"); int dday =
		 * c.getInt("dday");
		 * 
		 * for(int j=0; j<arrMyFriendList.size(); j++) { if
		 * (phone.equals(arrMyFriendList.get(j).getPhone())) { if (dday > -3) {
		 * arrNewsFeedList.add(name + "님의 " + title + " 이벤트가 시작되었습니다. (NEW)"); }
		 * } }
		 * 
		 * }
		 * 
	...
	public void GetFriendsWish() {
		...
		for (int k = 0; k < arrMyFriendList.size(); k++) {
			...
			try {
				JSONObject jObj = new AsyncFriendsWish().execute(
						"http://naddola.cafe24.com/getFriendWish.php?phone="
								+ mData.getPhone()).get();
				...
				if (jObj != null) {
					...
					for (int i = 0; i < friendsWish.length(); i++) {
						...
						if (!checkFWishlistData(phone)) {
							insertFWishlistData(id, phone, title, price, wish,
									date, imagePath, bookmark, event);
						}
					}
				}
			} catch (Exception e) {
